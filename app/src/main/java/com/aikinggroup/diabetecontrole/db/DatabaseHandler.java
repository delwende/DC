...
public class DatabaseHandler {
    ...
    private int realmVersion=5;

    public DatabaseHandler(Context context) {
        this.mContext = context;
        Realm.init(context);
        Constants.app = new App(new AppConfiguration.Builder(appId)
                .appName("DIABETECONTROLE")
                .build());
        this.realm = getNewRealmInstance();
    }
    public DatabaseHandler(Context context, Boolean login) {
        this.mContext = context;
        Realm.init(context);
        Constants.app = new App(new AppConfiguration.Builder(appId)
                .appName("DIABETECONTROLE")
                ...
                .build());
        this.realm = getNewRealmInstanceSync();
    }
    ...
    public Realm getNewRealmInstance() {
        if (mRealmConfig == null) {
            ...
            mRealmConfig = new RealmConfiguration.Builder()
                  ...
                  //  .migration(new Migration(new Class[] { UserEntity.class, GlucoseReading.class, HB1ACReading.class,PressureReading.class }))
                    .build();
        }
        return Realm.getInstance(mRealmConfig); // Automatically run migration if needed
    }
    public Realm getNewRealmInstanceSync() {
        if (mRealmConfig == null) {
    ...
    public RealmAsyncTask getRealmInstanceA() {
        realm.close();
         mRealmConfig = new SyncConfiguration.Builder(app.currentUser())
                .allowQueriesOnUiThread(true)
                .allowWritesOnUiThread(true).build();
        return Realm.getInstanceAsync(mRealmConfig, new Realm.Callback() {
            @Override
    ...
    public void addUser(UserEntity user) {
        realm.executeTransactionAsync(new Realm.Transaction() {
            ...
            public void execute(Realm realm) {
                ...
                realm.createObjectFromJson(UserEntity.class, json);
    ...
    public UserEntity getUser(Realm realm, long id) {
        return realm.where(UserEntity.class)
                .equalTo("_id", id)
                .findFirst();
    }
